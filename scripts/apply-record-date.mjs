/**
 * Adds record_date column and backfills existing rows.
...
if (!dbPassword || !projectRef) {
  ...
  console.error("Or run supabase-add-record-date.sql manually in Supabase SQL Editor.");
  process.exit(1);
}

const sqlPath = path.join(root, "supabase-add-record-date.sql");
...
console.log("Adding record_date column and backfilling…");
await client.connect();
await client.query(sql);
await client.end();
console.log("record_date ready. Run: npm run verify");
